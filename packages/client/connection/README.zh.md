...
部署在 loopback reverse proxy 后面的实例可以启用 `trustProxyAuth`。代理自己的 `auth_request` 成功后会覆盖 `X-DSH-Proxy-Auth: 1`；Connection 只在 socket peer 是 `127.0.0.1`、`::1` 或映射后的 loopback 时，才为 index、API 与 WebSocket 请求接受该声明。此模式提供的页面可以管理 Host 设置，因此模型提供方、插件配置与预设通过已认证代理仍然可用。该选项默认关闭，关闭时原生 token 流程保持不变。
