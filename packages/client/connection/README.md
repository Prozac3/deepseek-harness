...
Deployments behind a loopback reverse proxy can enable `trustProxyAuth`. After the proxy's own `auth_request` succeeds, it overwrites `X-DSH-Proxy-Auth: 1`; Connection accepts that assertion for index, API, and WebSocket requests only when the socket peer is `127.0.0.1`, `::1`, or mapped loopback. The served page can manage Host settings in this mode, so model providers, plugin configuration, and presets remain available through the authenticated proxy. The option is disabled by default, and the native token flow remains unchanged when it is off.
